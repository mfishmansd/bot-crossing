...
test('every registered harness implements the interface, and only Claude Code can write', () => {
  for (const h of HARNESSES) {
    ...
    // The one write the project allows is the archived flag, and only where the harness keeps
    // one. Everything else leaves `setArchived` out rather than stubbing it — see DECISIONS.md.
    if (h.id === 'claude-code') assert.equal(typeof h.setArchived, 'function')
    else assert.equal(h.setArchived, undefined, `${h.id} must not write to its harness`)
  }
})
...
test('a transcript with no turn markers is judged by when it last moved', async () => {
  // The older corpus carries no markers at all, and neither does a thread still inside its very
  // first turn. The file's own clock tells the two apart: the old one has not moved in months.
  const records = [
    askedFor('<user_query>old thread</user_query>'),
    { role: 'assistant', message: { content: [{ type: 'text', text: 'done' }] } },
  ]
  const home = await fakeCursor('tmp', records)
  const file = path.join(home, 'tmp', 'agent-transcripts', SESSION_ID, `${SESSION_ID}.jsonl`)
  const h = await cursorWith(home)
  assert.equal((await h.scanThreads())[0].running, true, 'written seconds ago and no turn closed yet')
  const hourAgo = new Date(Date.now() - 60 * 60 * 1000)
  await fsp.utimes(file, hourAgo, hourAgo)
  assert.equal((await h.scanThreads())[0].running, false, 'an hour old is not still working')
  await fsp.rm(home, { recursive: true, force: true })
})
...
test('Cursor opens the workspace a thread ran in, and refuses one it cannot place', async () => {
  const home = await fakeCursor('tmp', [askedFor('<user_query>hi</user_query>')])
  const h = await cursorWith(home)
  // No per-thread link exists, so a thread is handed back as its folder — and without one there
  // is nothing to open.
  assert.equal(h.openThread({ sessionId: SESSION_ID }).ok, false)
  const opened = h.newSession('/tmp/some repo')
  assert.equal(opened.ok, true)
  if (opened.command) {
    // `cursor <dir>` from PATH, or the OS opener with the app named: the folder is the last word.
    assert.equal(opened.command[opened.command.length - 1], '/tmp/some repo')
  } else {
  ...
  assert.equal(h.openThread({ sessionId: SESSION_ID, cwd: 'relative/path' }).ok, false)
  await fsp.rm(home, { recursive: true, force: true })
})
