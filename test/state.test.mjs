...
test('archiving without a harness record is recorded here and says so', async () => {
  await withServer(async ({ call }) => {
    const res = await call('/api/archive', { method: 'POST', body: JSON.stringify({ id: 'x', archived: true }) })
    assert.equal(res.status, 200)
    const body = await res.json()
    assert.equal(body.ok, true)
    assert.equal(body.harnessRecord, false, 'no ref, so nothing in any harness was touched')
    // The colony's list is the page's to write; the endpoint never touches colony.json itself.
    const state = await (await call('/api/state')).json()
    assert.deepEqual(state.archived, [])
    const bad = await call('/api/archive', { method: 'POST', body: JSON.stringify({ archived: true }) })
