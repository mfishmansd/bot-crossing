...
export function mergeState(base, local, remote) {
  ...
  return {
    ...
    // The later visit is the true one whichever tab made it: it only ever moves forward.
    lastDeckVisit: Math.max(Number(l.lastDeckVisit) || 0, Number(r.lastDeckVisit) || 0),
